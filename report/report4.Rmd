...
To conclude, we have seen that Prime Video has the highest count of movies but is not the platform with the highest rate of good movies with respect to IMDb and Rotten Tomatoes scores. Also, these platform concerns different type of movies (genre and age categories) which imply that one platform is better if we are more interested in a certain type of movie and another in a different case.

We also saw that there is similarity between each of these platforms on the score of overall movies independently of other criteria, thus the choice of one or another is subjective.
